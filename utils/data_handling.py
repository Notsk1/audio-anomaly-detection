...
def split_data():
    ...
    for id_folder in list(root_object_path.iterdir()):
        ...
        abnormal_validation_split = abnormal_file_numbers[int(len(abnormal_files_list)*0.4):]
...
def serialize(mel_or_stft):
    """
    Calculate mel spectrogram and serialize the results

    Arguments:
    - mel_or_stft: Define which method is used for extracting audio feature
    """
    ...
    for id_folder in list(root_object_path.iterdir()):
        ...
        for abnormal_sub_folder in abnormal_folder_list:
            for abnormal_file in list(abnormal_sub_folder.iterdir()):
                ...
                elif mel_or_stft == 'stft':
                    audio_features = extract_stft(audio_data)
                ...
                new_path = abnormal_file.absolute().as_posix().replace('raw/fan', f'serialized/fan/{mel_or_stft}')
                pathlib.Path(new_path).parents[0].mkdir(parents=True, exist_ok=True)
                file = open(new_path.replace(f'{abnormal_file.stem}.wav', f'{abnormal_file.stem}_{mel_or_stft}.pickle'), 'wb')
        ...
        for normal_sub_folder in normal_files_list:
            for normal_file in list(normal_sub_folder.iterdir()):
                ...
                # Get new path and make parent directories if necessary
                new_path = normal_file.absolute().as_posix().replace('raw/fan', f'serialized/fan/{mel_or_stft}')
                pathlib.Path(new_path).parents[0].mkdir(parents=True, exist_ok=True)
                file = open(new_path.replace(f'{normal_file.stem}.wav', f'{normal_file.stem}_{mel_or_stft}.pickle'), 'wb')
                pickle.dump(features_and_classes, file)
                file.close()

def extract_stft(audio_file, n_fft = 1024, hop_length = 512):
    """Extracts and returns the STFT from the 'audio_file' audio file"""
    return stft(y=audio_file, n_fft=n_fft, hop_length=hop_length)
